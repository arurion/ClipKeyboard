...
- システムのコピー操作を自動で監視し、履歴として保存(ピン留め・検索・削除)。
  テキストだけでなく画像・PDF・ZIPなど任意ファイルにも対応
- キーボード上からタップでそのままペースト(テキストは`commitText`、対応
  エディタでは画像を`commitContent`でリッチコンテンツとして直接挿入)
- 長押しでピン留め切替・編集・削除を選べるクイックメニュー(IME内蔵の
  Viewオーバーレイ。ダイアログは使わずWindow Token不要のため安全)
- 保存済みクリップのテキスト編集・ラベル付け(ホストアプリの編集画面を開いて編集。
  3,000字を超える長文は自動でページ分割される)
- メイン画面のFAB・キーボード上の「＋」から手動で定型文を新規作成
- 任意ファイルのインポート(SAFのファイル選択、画像・バイナリはアプリ内保存)
- 配色・角丸・文字サイズなどテーマの編集(ライブプレビュー・カラーチップ付き、プリセット3種)
- 直前のキーボードへ確実に戻れる3段フォールバック付きのIME切替ボタン
- キーボード最下部のエディットバー(元に戻す/やり直す・カーソル移動・1文字削除・改行)
- Gboardなど他のキーボードでコピーした内容もキーボード起動時に即時反映
