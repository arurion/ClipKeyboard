...
- 配色・角丸・文字サイズなどテーマの編集(プリセット3種 + 個別カラーコード編集)
...
2. 環境変数 (`RELEASE_STORE_FILE`, `RELEASE_STORE_PASSWORD`, `RELEASE_KEY_ALIAS`, `RELEASE_KEY_PASSWORD`)
3. 上記が無ければ **デバッグ鍵に自動フォールバック**し、ビルド自体は失敗させません
   (ログに警告を出します)。

GitHub Actions (`.github/workflows/release.yml`) は `main` ブランチへの push、
`v*` タグの push、Pull Request(main向け)のいずれでも自動的に `assembleRelease`
を実行します。ただし GitHub Release への添付は `v*` タグ push の場合のみです
(通常pushではビルド確認のみ行い、Actionsの Artifacts からAPKをダウンロードできます)。
リポジトリのデフォルトブランチ名が `main` 以外(例: `master`)の場合は
`.github/workflows/release.yml` の `branches: [ main ]` を書き換えてください。
`RELEASE_STORE_BASE64` 等のリポジトリシークレットが設定されていれば本番署名で、
無ければデバッグ署名でAPKをビルドします。

必要なシークレット例:

| Secret name              | 内容                              |
|---------------------------|-----------------------------------|
| `RELEASE_STORE_BASE64`    | `.jks` を `base64` エンコードした文字列 |
